...
Set the menu activation key in `settings.ini` (AddonWeapons folder).

# Weapon components (for mod authors)
To display weapon components in the menu, fill in `shop_weapon.meta` correctly.
...
If you are not adding all the tints in your mod, or combining them from different types of weapons, you will need to create a text file in AddonWeapons/tints with the name of the weapon from `shop_weapon.meta`, and write all the tints names line by line. Otherwise, the menu will display tints without names (Tint 1, Tint 2, etc.), or the names will not correspond to the actual weapon tint.
